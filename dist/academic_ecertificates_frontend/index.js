...
const academic_ecertificates_backend = createActor(canisterId);
...
function redirect(path, params) {
  ...
  myUrlWithParams.searchParams.append('canisterId', "c2lt4-zmaaa-aaaaa-qaaiq-cai");
  if (params) {
  ...
  if (resp.statusCode === BigInt(200)) {
    ...
    $("i[data-id]").on('click', async function (e) {
      ...
      if (resp.statusCode == BigInt(200)) {
        var baseurl =  false
          ? 0
          : `http://127.0.0.1:4943/?canisterId=${"c2lt4-zmaaa-aaaaa-qaaiq-cai"}&cert=`;
        var universityUrl =  false
          ? 0
          : `http://127.0.0.1:4943/university.html?canisterId=${"c2lt4-zmaaa-aaaaa-qaaiq-cai"}&principal=`;
...
async function universityinit() {
  ...
  if (url.searchParams.has('principal')) {
    ...
    var resp = await academic_ecertificates_actor.getUniversity([_dfinity_principal__WEBPACK_IMPORTED_MODULE_3__.Principal.fromText(universityPrincipal)]);
    console.log(resp);
    if (resp.statusCode === BigInt(200)) {
      var university = resp.university[0];
      var certificates = resp.certs[0];
